filename = "20160622_ll_60s_05.csv"
system.name = "LL"
...
for(c in 1:nrow(ranges)){
  ...
  sprintf("--------------------------------------------------------------")
  sprintf("RANGE:\n %d ns - %d ns", range.start, range.end)
  ...
  range.index = (1:data.ll.length)[range.target]
  range.index.step = range.index[2:length(range.index)] - range.index[1:length(range.index)-1]
  text.capital = sprintf("Step of Internal Latency on %s(%d ns ~ %d ns)", system.name, range.start, range.end)
  text.comment = sprintf("Step mean=%f, sd=%f", mean(range.index.step), sd(range.index.step))
  hist(range.index.step,breaks=100, 
       main = text.capital, 
       sub = text.comment,
       xlab = "How many packets between two samples in this range", ylab="Pakcet number")
}